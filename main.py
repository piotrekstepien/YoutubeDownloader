...
layout = [[sg.TabGroup([[sg.Tab("info", info_tab), sg.Tab("download", download_tab)]])]]
window = sg.Window("YTDownloader", start_layout)

video_link = "https://www.youtube.com/watch?v=NXX-I7N_a0k"
...
while True:
    ...
    if event == "Submit":
        ...
        # video info:
        length_min = math.floor(video_object.length / 60)
        length_sec = video_object.length % 60
        window = sg.Window("YTDownloader", layout, finalize=True)
        window["-TITLE-"].update(video_object.title)
        window["-LENGTH-"].update(f"{length_min},{length_sec} minutes")
